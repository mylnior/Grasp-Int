...
def forward_pass_method(model, im, args):
    output = model(im, **args)
    return output
class RigidObjectPredictor:
    def __init__(self,
        ...
        self.it =0
    def predict(self, image,  detector_kwargs=None):        
        ...
        with torch.no_grad():
            ...
            os.system('nvidia-smi | grep python')
            win_images = torch.as_tensor(np.stack([image[0][w[0][1] : w[1][1], w[0][0] : w[1][0]] for w in self.windows] )).permute(0, 3, 1, 2).cuda().float()/ 255
            ...
            # exit()
            if self.detect:
                detections_full = self.detector(win_images, **detector_kwargs)
...
class KnownObjectDetector:
    def __init__(self, device, cam_setting_path = _DEFAULT_CAM_SETTINGS_PATH, cam_info_path=_DEFAUL_CAM_INTRINSICS,
     dataset = 'tless', render_txt =False, render_overlay = False, render_bboxes = True):
    ...
    def get_objects(self, image, detection_threshold=0.8):
