...
class RigidObjectPredictor:
    def __init__(self,
        ...
        self.detector_kwargs2 = detector_kwargs
        ...
        self.it =0
        self.on = True
        self.new_image_full = False
        self.new_images_windows = False
        self.p_full = mp.Process(target=self.forward_pass_full_detector)
        self.p_win = mp.Process(target=self.forward_pass_windows_detector)
        self.p_win.start()
        self.p_full.start()

    def set_images(self,images):
        self.image_full = images['image_full']
        self.images_windows = images['images_windows']
        self.new_image_full = True
        self.new_images_windows = True

    def forward_pass_full_detector(self):
        while self.on:
            if self.new_image_full:
                self.detections = self.detector(self.image_full,self.detector_kwargs)
                self.new_image_full = False
                self.image_full_processed = True
            else:
                time.sleep(0.02)
                continue
    ...
    def forward_pass_windows_detector(self):
        while self.on:
            if self.new_images_windows:
                self.detections_windows = self.detector_windows(self.images_windows,self.detector_kwargs2)
                self.new_images_windows = False
                self.images_windows_processed = True
            else:
                time.sleep(0.02)
                continue
    ...
    def predict(self, image):        
        ...
        os.system('nvidia-smi | grep python')
        new_image = {}
        new_image['images_windows'] = torch.as_tensor(np.stack([image[0][w[0][1] : w[1][1], w[0][0] : w[1][0]] for w in self.windows] )).permute(0, 3, 1, 2).cuda().float()/ 255
        
        os.system('nvidia-smi | grep python')
        new_image['image_full'] = torch.as_tensor(np.stack(image)).permute(0, 3, 1, 2).cuda().float() / 255
        ...
        # exit()
        if self.detect:
            self.set_images(new_image)

            while not (self.image_full_processed and self.images_windows_processed):
                time.sleep(0.01)
