...
class GraspingDetector:
    def __init__(self, device_name='OAK', hand_detection='mediapipe',  object_detection='cosypose') -> None:
        ...
        self.detect_img_queue = mp.Queue(maxsize=1)
    ...
    def detect_task(self, img):
        print('DETECT')
        self.object_detections_queue.put(self.object_detector.detect(img))
    ...
    def estimate_task(self,img):
        if self.object_detections_queue.full():
            det = self.object_detections_queue.get()
            print(det)
        else:
            det = None
        self.objects_pose = self.object_pose_estimator.estimate(img, detections = det)
        self.estimate_task_done = True
