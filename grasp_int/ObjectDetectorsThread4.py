...
class RigidObjectPredictor:
    def __init__(self,
        ...
        os.system('nvidia-smi | grep python')
        self.detector_windows = [load_detector(object_detector_run_id) for w in self.windows]
        os.system('nvidia-smi | grep python')
    ...
    def forward_pass_windows_detector(self, index):
        self.detections_windows = self.detector_windows[index](self.images_windows,**self.detector_kwargs)
    ...
    def predict(self, image):        
        ...
        with torch.no_grad():
            ...
            if self.detect:
                ...
                self.p_win = [threading.Thread(target=self.forward_pass_windows_detector, args= (i, )) for i in range(len(self.windows))]
                self.p_full.start()
                for p in self.p_win:
                    p.start()
